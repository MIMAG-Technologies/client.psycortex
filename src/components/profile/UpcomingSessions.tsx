...
import { getUserChatSessions, getUserVideoSessions, getUserOfflineSessions } from "@/utils/user";
...
const UpcomingSessions: React.FC<SessionHistoryProps> = ({ userId }) => {
    ...
    useEffect(() => {
        const fetchSessions = async () => {
            setIsLoading(true);
            try {
                const [chats, videos] = await Promise.all([
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            const allSessions = [...chatSessions, ...videoSessions, ...offlineSessions];
            ...
            <div className="border rounded-lg p-4 sm:p-5 hover:bg-gray-50 transition-all shadow-sm hover:shadow-md">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                    <div className="flex items-start">
                        {session.counsellor && (
                            <img
                                src={session.counsellor.image || "/user-dummy-img.png"}
